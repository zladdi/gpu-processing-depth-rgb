...
//	g_Capture.nodes[CAPTURE_IMAGE_NODE].captureFormat = XN_CODEC_JPEG;
	g_Capture.nodes[CAPTURE_IMAGE_NODE].captureFormat = CODEC_DONT_CAPTURE;
...
//	g_Capture.nodes[CAPTURE_AUDIO_NODE].captureFormat = XN_CODEC_UNCOMPRESSED;
	g_Capture.nodes[CAPTURE_AUDIO_NODE].captureFormat = CODEC_DONT_CAPTURE;
}
...
void captureBrowse(int)
{
	...
	time(&time_now);
	now = gmtime(&time_now);
	sprintf(fileName, "/mnt/Captured_%d-%d-%d_%d-%d_.oni", 1900+now->tm_year, now->tm_mon, now->tm_mday, now->tm_hour, now->tm_min);
	strcpy(g_Capture.csFileName, fileName);
#endif
